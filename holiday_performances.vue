...
<script>
    define(["Vue", "vuex", "jquery", "vue-meta", "lightbox", "vue-lazy-load"], function (Vue, Vuex, jQuery, Meta, Lightbox, VueLazyload) {
        ...
        Vue.use(Lightbox);
        Vue.use(VueLazyload);
        return Vue.component("holiday-performance-component", {
            ...
            computed: {
                ...
                images() {
                    var images = this.findRepoByName("Holiday Performances").images
                    if(images != undefined){
                        return images
                    }
                },
            ...
            methods: {
                loadData: async function () {
                    try {
                        let results = await Promise.all([this.$store.dispatch("getData", "repos"), this.$store.dispatch('LOAD_PAGE_DATA', {url: this.property.mm_host + "/pages/northpark-holiday-performances.json"})]);
                        return results;
                    } catch (e) {
